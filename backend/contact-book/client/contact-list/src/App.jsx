...
function App() {
  ...
  const handleSubmit = (e) => {
    ...
    if (editId) {
         axios.put(`${apiBaseUrl}update/${editId}`,formData)
         .then((res) => res.data)
        .then((finalRes) => {
          if (finalRes._status) {
            Swal.fire({
              title: "Data Update!",
              text: "new enquiry Update",
              icon: "success",
            });
            getEnquiry();
            // e.target.reset()
  ...
  let getDetails = (id) => {
    setEditId(id);
    axios.get(`${apiBaseUrl}view/${id}`)
      .then((res) => res.data)
      .then((finalRes) => {
        let {studentName,studentEmail,studentPhone}=finalRes.data
       ;
           setFormData({
              studentName,
              studentEmail,
                  ...
                  return (
                    <tr>
                      ...
                      <th className="px-6 py-3 text-left text-xs font-medium text-gray-700 uppercase tracking-wider border-b">
                        ...
                        |
                        <button
                          onClick={() => getDetails(obj._id)}
                          className="bg-green-500 rounded-[10px] p-2 text-white"
                        >
                          Edit
                        </button>
                      </th>
